...
public partial class SettingsWindow : Window
{
    ...
    private async void TestButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        var result = await _describer.TestConnectionAsync(candidate);
        SetupStatus.Text = result;
    }
    ...
    private AppSettings BuildCandidate()
    {
        ...
        return new AppSettings
        {
            UseLocalAi = local,
            AllowRemoteAi = !local,
            SendScreenshotsToAi = !local && VisionCheck.IsChecked == true && AiProviderCatalog.IsVisionModel(provider.Id, model),
    ...
    {
        if (_firstRun && !_settings.HasCompletedAiSetup)
            _settings.HasCompletedAiSetup = false;
        DialogResult = false;
    }
}
